...
function log_80211ax_info ()
{
	...
	if [ "$RXHEMU" -gt "0" ]; then
		# RXMURATIO, RXOFDMA_RATIO, RXRU_242ORLESS and RXRU_484ORMORE are percentage value and
		# limited to range 0-10.
		# RXMURATIO: RX MU packets in all RX packets. This value is float to provide more accuracy
		# RXOFDMA_RATIO: RX OFDMA packets in all RX MU packets
		# RXRU_242ORLESS: RX packets of 242-tone or smaller RU in all RX OFDMA packets
		# RXRU_484ORMORE: RX packets of 484-tone or bigger RU in all RX OFDMA packets
		RXMURATIO=$(echo "$RXHEMU $RXHESU" | awk '{printf "%.2f \n", $1*10/($1+$2)}')
