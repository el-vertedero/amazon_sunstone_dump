...
if [ -e /sys/fs/pstore/console-ramoops-0 ]; then
	...
	cat /sys/fs/pstore/console-ramoops-0 >> $LASTKMSG
	if [ -e /proc/life_cycle_reason ]; then
		cat /proc/life_cycle_reason >> $LASTKMSG
	fi
else
	echo "/sys/fs/pstore/console-ramoops-0 does not exist!" >> $LASTKMSG
fi
...
if [ -b /dev/block/by-name/expdb ]; then
	dd if="/dev/block/by-name/expdb" of=$FILE_PATH/expdb bs=4m
	mkfs.ext4 /dev/block/by-name/expdb
else
	echo "expdb partition does not exist!" >> $FILE_PATH/$MEMDUMP_LOG
fi
